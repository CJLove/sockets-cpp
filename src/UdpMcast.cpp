...
void UdpMcast::publishDisconnected(const SocketRet &ret) {
    if (m_callback) {
        m_callback->onDisconnect(ret);
    }
}

void UdpMcast::ReceiveTask() {
    while (!m_stop) {
        if (m_fd != -1) {
...
SocketRet UdpMcast::sendMsg(const unsigned char *msg, size_t size)
{
    SocketRet ret;
